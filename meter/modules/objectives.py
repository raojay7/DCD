...
from meter.modules.KD_loss import distillation_loss
from meter.modules.KD_loss import WSLDistiller
...
def compute_irtr(pl_module, batch):
    ...
    if pl_module.useTeacher and phase == "train":

        teacher_infer = pl_module.teacher_infer(
            {
        ...
        teacher_score = pl_module.rank_output(teacher_infer["teacher_feats"])[:, 0]
        teacher_score = rearrange(teacher_score, "(bs fs) -> bs fs", bs=_bs, fs=false_len + 1)


        if pl_module.useHNM:
            hard_negative = 7
            value, neg_idxs = torch.sort(teacher_score, descending=True)
            neg_idxs = neg_idxs[:, :hard_negative + 1]
            # scores rank
            flag=False
            for i in range(_bs):
                # print(neg_idxs)
                if neg_idxs[i][0]!=0:
                    flag=True
            teacher_score = teacher_score[torch.arange(teacher_score.shape[0])[:, None], neg_idxs]

            # Positive contains 0
            zero_mask = (~neg_idxs.to(torch.bool)).to(torch.float)
            # print(zero_mask)
            lie = torch.ones(neg_idxs.size(-1), 1).to(teacher_score)
            has_zero = zero_mask.mm(lie)  # tensor([[0.],[1.]])
            # 0 to last column
            one_padding = torch.ones([_bs, hard_negative]).to(teacher_score)
            new_mask = torch.cat([one_padding, has_zero], dim=1)
            neg_idxs = neg_idxs * new_mask

            # print(neg_idxs)
            # select 0
            zero_mask = neg_idxs.to(torch.bool)
            other_tensor = neg_idxs[zero_mask].view(neg_idxs.size(0), -1)
            # print(other_tensor)
            zero_padding = torch.zeros([neg_idxs.size(0), 1]).to(teacher_score)

            # print(zero_padding)
            student_to_learn = torch.cat([zero_padding, other_tensor], dim=1).to(teacher_score).to(torch.long)

            # sample selection
            images = images[torch.arange(images.shape[0])[:, None], student_to_learn]
            text_ids = text_ids[torch.arange(text_ids.shape[0])[:, None], student_to_learn]
            text_masks = text_masks[torch.arange(text_masks.shape[0])[:, None], student_to_learn]
            text_labels = text_labels[torch.arange(text_labels.shape[0])[:, None], student_to_learn]
            infer = pl_module.infer(
                {
                    "image": [rearrange(images, "bs fs c h w -> (bs fs) c h w")],
                    "text_ids": rearrange(text_ids, "bs fs tl -> (bs fs) tl"),
            ...
            score = pl_module.stu_rank_output(infer["cls_feats"])[:, 0]
            score = rearrange(score, "(bs fs) -> bs fs", bs=_bs, fs=hard_negative + 1)
            answer = torch.zeros(_bs).to(score).long()
            alpha = 0.2
            irtr_loss, d_loss, nll_loss = distillation_loss(score, answer, teacher_score, 1 , alpha,hard_negative,pl_module.global_step,flag)
            # irtr_loss, d_loss, nll_loss = WSLDistiller(score, answer, teacher_score, 1, alpha, hard_negative,
            #                                        pl_module.global_step, flag)
        else:
            infer = pl_module.infer(
                {
                    "image": [rearrange(images, "bs fs c h w -> (bs fs) c h w")],
            ...
            answer = torch.zeros(_bs).to(score).long()
            alpha = 0.2
            irtr_loss, d_loss, nll_loss = distillation_loss(score, answer, teacher_score, 1, alpha, false_len,
                                                            pl_module.global_step, False)
            # irtr_loss, d_loss, nll_loss = WSLDistiller(score, answer, teacher_score, 1 , alpha,hard_negative,pl_module.global_step,flag)
        pl_module.log(f"irtr/{phase}/d_loss", d_loss)
        pl_module.log(f"irtr/{phase}/nll_loss", nll_loss)

        # 4 scores
        pl_module.log(f"irtr/{phase}/teacher_score1", torch.mean(teacher_score[:, 0]))
        pl_module.log(f"irtr/{phase}/student_score1", torch.mean(score[:, 0]))
        pl_module.log(f"irtr/{phase}/teacher_score2", torch.mean(teacher_score[:, 1]))
        pl_module.log(f"irtr/{phase}/student_score2", torch.mean(score[:, 1]))
        pl_module.log(f"irtr/{phase}/teacher_score3", torch.mean(teacher_score[:, 2]))
        pl_module.log(f"irtr/{phase}/student_score3", torch.mean(score[:, 2]))
        pl_module.log(f"irtr/{phase}/teacher_score4", torch.mean(teacher_score[:, 3]))
        pl_module.log(f"irtr/{phase}/student_score4", torch.mean(score[:, 3]))
    ...
    for img_batch in tqdm.tqdm(image_preload, desc="rank loop"):
        ...
        for txt_batch in text_preload:
            ...
            with torch.cuda.amp.autocast():
                score = pl_module.stu_rank_output(
                    pl_module.infer(
                        {
