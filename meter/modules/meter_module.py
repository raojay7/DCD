...
class METERTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        # student
        self.useTeacher = self.hparams.config["useTeacher"]
        self.useSucc = self.hparams.config["useSucc"]
        self.useHNM = self.hparams.config["useHNM"]
        self.stu_cross_modal_image_layers = nn.ModuleList([BertCrossLayer(bert_config) for _ in range((int)(config['num_top_layer']/2))])
        self.stu_cross_modal_image_layers.apply(objectives.init_weights)
        self.stu_cross_modal_text_layers = nn.ModuleList([BertCrossLayer(bert_config) for _ in range(int(config['num_top_layer']/2))])
        self.stu_cross_modal_text_layers.apply(objectives.init_weights)
        self.stu_cross_modal_image_pooler=heads.Pooler(config["hidden_size"])
        self.stu_cross_modal_image_pooler.apply(objectives.init_weights)
        self.stu_cross_modal_text_pooler = heads.Pooler(config["hidden_size"])
        self.stu_cross_modal_text_pooler.apply(objectives.init_weights)
        ...
        if self.hparams.config["loss_names"]["irtr"] > 0:
            ...
            self.margin = 0.2
            self.stu_rank_output = nn.Linear(hs, 1)  # stu
            self.stu_rank_output.weight.data = self.itm_score.fc.weight.data[1:, :]
            self.stu_rank_output.bias.data = self.itm_score.fc.bias.data[1:]
        ...
        if (
            ...
            if self.useSucc and self.useTeacher:
                idx = [0, 3, 5]

                if self.hparams.config["resume_from"] == None:
                    scc_image_layers = []
                    scc_text_layers = []
                    for ix in range(len(idx)):
                        # initialization
                        scc_image_layers.append(deepcopy(self.cross_modal_image_layers[idx[ix]]))
                        scc_text_layers.append(deepcopy(self.cross_modal_text_layers[idx[ix]]))
                    self.stu_cross_modal_image_layers = nn.ModuleList(scc_image_layers)
                    self.stu_cross_modal_text_layers = nn.ModuleList(scc_text_layers)

                    self.stu_cross_modal_text_pooler=deepcopy(self.cross_modal_text_pooler)
                    self.stu_cross_modal_image_pooler = deepcopy(self.cross_modal_image_pooler)
                    # self.stu_rank_output = deepcopy(self.rank_output)
            # 3.fix
            self.fixModel()
            if self.hparams.config["loss_names"]["irtr"] > 0:
                for p in self.rank_output.parameters():
                    p.requires_grad = False
    ...
    def fixModel(self):
        ...
        for p in self.token_type_embeddings.parameters():
            p.requires_grad = False
        for p in self.cross_modal_text_transform.parameters():
            p.requires_grad = False
        for p in self.cross_modal_image_transform.parameters():
            p.requires_grad = False
        for p in self.cross_modal_image_layers.parameters():
            p.requires_grad = False
        for p in self.cross_modal_text_layers.parameters():
            p.requires_grad = False
        for p in self.vit_model.parameters():
            p.requires_grad = False
        for p in self.text_transformer.parameters():
            p.requires_grad = False
        for p in self.cross_modal_image_pooler.parameters():
            p.requires_grad = False
        for p in self.cross_modal_text_pooler.parameters():
            p.requires_grad = False
    @torch.no_grad()
    def teacher_infer(
        self,
    ...
    def infer(
        ...
        x, y = text_embeds, image_embeds
        # 学生的推理的不同
        for text_layer, image_layer in zip(self.stu_cross_modal_text_layers, self.stu_cross_modal_image_layers):
            x1 = text_layer(x, y, extend_text_masks, extend_image_masks)
            y1 = image_layer(y, x, extend_image_masks, extend_text_masks)
            x, y = x1[0], y1[0]

        text_feats, image_feats = x, y
        cls_feats_text = self.stu_cross_modal_text_pooler(x)
        if self.is_clip:
            cls_feats_image = self.stu_cross_modal_image_pooler(y)
        else:
            avg_image_feats = self.avgpool(image_feats.transpose(1, 2)).view(image_feats.size(0), 1, -1)
            cls_feats_image = self.stu_cross_modal_image_pooler(avg_image_feats)
        cls_feats = torch.cat([cls_feats_text, cls_feats_image], dim=-1)
